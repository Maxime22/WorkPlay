...
describe('AddActivity', () => {
    ...
    it('opens the modal when the button is pressed', () => {
        const { getByText, getByPlaceholderText } = render(<AddActivity addInput={() => {}} />);
        fireEvent.press(getByText('+'));
        expect(getByPlaceholderText('Enter Title')).toBeTruthy();
    });

    it('closes the modal when the close button is pressed', () => {
        const { getByText, queryByPlaceholderText } = render(<AddActivity addInput={() => {}} />);
        fireEvent.press(getByText('+'));
        fireEvent.press(getByText('×'));
    ...
    it('calls addInput with the correct title and closes the modal', () => {
        ...
        const { getByText, getByPlaceholderText } = render(<AddActivity addInput={addInputMock} />);
